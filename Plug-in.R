...
drawPoints = function(xy1, xy2) {
  x = rbind(xy1, xy2)
  colors <- c("setosa" = "red", "virginica" = "blue")
  plot(x[, 1:2], pch = 21, bg = colors[iris$Species], col = colors[iris$Species], asp = 1,main =  'Подстановочный алгоритм (plug-in)', xlab = "Длина лепестка", ylab = "Ширина лепестка")
}

xl <- iris[, 3:5]

class1 <- xl[xl$Species=='setosa',]
class2 <- xl[xl$Species=='virginica',]
